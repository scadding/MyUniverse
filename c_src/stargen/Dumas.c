...
//
// Just as Edmond Dant�s disguised himself to catch the disloyal M. Danglars, so
...
// DUMAS seeks and punishes those who doubly deallocate. Where Dant�s became
// the Count of Monte-Cristo, Sinbad the Sailor, Abb� Busoni, and Lord Wilmore,
// DUMAS becomes malloc, calloc, realloc and free. As Dant�s was guarded in
// the Ch�teau d'If, so each allocated item is protected by a guard and a sentinel
// to detect corruption (overwriting from below) and a wall to protect against
// escaping (writing beyond the end of the item). And as Dant�s' mentor kept a
...
// The Ch�teau d'If structure, to keep track of each allocated chunk of memory.
...
//	field of the Ch�teau d'If structure. The result is a comma-seperated list of
