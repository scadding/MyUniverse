...
# yourselves out)
#
# Micki Kaufman: Created this knocked-around version for my
# sector LBB pdf script. Used for generating borders, regions, routes 
# and labels.
# 
...
map-background-line 0

map-route-line 20
map-route-stroke gray

map-region-line 12
map-region-filled-fill lightgrey
...
map-label-pos 0 12
map-label-fill dimgray
map-label-large-size 70
map-label-size 55
map-label-small-size 40
EOF
}
...
sub drawroutes {
	...
	$pdf->savestate;
#	$pdf->rectxy($grid{origX},$grid{origY},$grid{origX} + $grid{width},		$grid{origY} + $grid{height});
#	$pdf->{$cliphack}->clip;
#	$pdf->{$cliphack}->endpath;
			styledcircle("jammy",0,1,1);
			styledcircle("map-route-stroke",0,1,1);
