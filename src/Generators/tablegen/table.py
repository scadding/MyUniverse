...
class tableDB:
    def __init__(self, table, genre, con):
        ...
        self.genre = genre
        cur = con.cursor()
        cur.execute("SELECT SubTableName, Type, Length FROM Tables WHERE TableName == \"%s\"" % (table))
        self.length = dict()
        self.type = dict()
...
class tableMgr:
    ...
    def parse(self, table, exp):
        ...
        ret1 = ret1 + ret[last:]
        last = 0
        q = QuotedString('%', multiline=False, unquoteResults=True, endQuoteChar='%')
        n = ''
